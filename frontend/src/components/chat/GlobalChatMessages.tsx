...
export const GlobalChatMessages = memo(({
  ...
  const selectionToolbarRef = useRef<{
    text: string;
    top: number;
    left: number;
  } | null>(null);
  ...
  const updateSelectionToolbar = useCallback((val: typeof selectionToolbar) => {
    selectionToolbarRef.current = val;
    setSelectionToolbar(val);
  }, []);

  const handleSelectionCheck = useCallback(() => {
    // 15ms debounce allows browser to finalize selection bounds
    setTimeout(() => {
      ...
      try {
        ...
        // Position below selection to avoid overlapping browser search popups (Opera GX, Edge)
        const showBelow = rect.top < 70 || (window.innerHeight - rect.bottom > 60);
        const top = showBelow ? rect.bottom + 10 : Math.max(12, rect.top - 46);
  ...
  // Dismiss toolbar immediately on scroll or resize to prevent laggy drag
  useEffect(() => {
    const handleDismiss = () => {
  ...
  const handleSelectionAction = useCallback((promptPrefix: string, text: string) => {
    updateSelectionToolbar(null);
    window.getSelection()?.removeAllRanges();
    sendMessage(`${promptPrefix}: "${text}"`);
  }, [sendMessage, updateSelectionToolbar]);

  return (
    <div className="relative flex-1 min-h-0 flex flex-col overflow-hidden">
      {/* Floating Selection Toolbar Popover (Fixed in viewport, perfectly positioned) */}
      {selectionToolbar && (
        <div
          className="fixed z-50 -translate-x-1/2 flex items-center bg-[#1c1c22] border border-white/20 shadow-2xl rounded-full p-1 text-xs text-white transition-opacity duration-150 animate-in fade-in zoom-in-95 pointer-events-auto"
        ...
        >
          <button
            ...
            onClick={(e) => {
              e.preventDefault();
              e.stopPropagation();
              handleSelectionAction("Ask Iris about", selectionToolbar.text);
            }}
          ...
          <button
            ...
            onClick={(e) => {
              e.preventDefault();
              e.stopPropagation();
              handleSelectionAction("Elaborate on", selectionToolbar.text);
            }}
          ...
          <button
            ...
            onClick={(e) => {
              e.preventDefault();
              e.stopPropagation();
              handleSelectionAction("Simplify", selectionToolbar.text);
            }}
            className="flex items-center gap-1.5 px-3 py-1 rounded-full hover:bg-white/10 text-white/80 hover:text-white transition-colors cursor-pointer"
          >
            <span>Simplify</span>
